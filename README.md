...
- **Map layers:** Terrain, Realms (political borders), Cultures, Races, and Resources (pick iron, tin, gold, ley lines and others). Trade routes are drawn brown for internal trade (heavier on the roads to each nation's hub), green for trade under an agreement, and dashed for sea lanes. Roads appear where traffic has worn them in. A diamond ◆ marks a capital, a triangle ▲ is an army on the march (its dashed line shows where it is heading), and gold dots ● are merchant caravans.
- **Nation view:** a full dossier on any nation. It shows what the nation makes, what it needs and what it is short of, its internal, treaty and caravan trade, trade agreements and partners, tariff, armies and wars, research progress, peoples and cultures (with their survival traits), rulers, settlements, population history and chronicle.
...
| **Trade: three kinds** | **Internal trade** moves goods freely between a nation's own settlements, along its neighbour roads and along a road from every town to the nation's hub (its largest settlement), so roads converge on the heart of the realm. **Trade agreements** are treaties signed by friendly neighbours whose economies complement each other. They open the border to direct trade, but only in goods the buying nation cannot produce enough of itself, so nations stay self-reliant and trade for what they lack. War tears agreements up, and souring relations let them lapse. **Free traders** are caravans owned by a town's merchant houses. Each looks for the most profitable cargo it can carry to any market within reach, travels there over one or more years (visible on the map), pays the border toll where there is no agreement, sells, buys a return cargo and comes home. Caravans can be seized by enemies at war or lost to bandits in the wilds. Rulers also collect tribute in grain, which feeds large capitals. |
...
src/engine/            deterministic simulation; no DOM, runs in Node or the browser
  ...
  systems/             territory, economy, trade, caravans, migration, culture, politics, military, technology, events, setup
  chronicle.ts         Markdown chronicle, summaries, JSON snapshot
src/ui/                browser app: map renderer, charts, panels, nation view, tech tree
...
A medium map (180×110) runs about 1,000 years in 30 seconds headless, with around 400 settlements, dozens of armies and hundreds of caravans over its history.
